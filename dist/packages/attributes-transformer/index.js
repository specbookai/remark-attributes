...
export function attributesTransformer(root) {
    visit(root, (node) => node.type === 'paragraph', (node, index, parent) => {
        if (node.children?.length === 1 && node.children[0].type === 'attrs') {
            const index = parent.children.indexOf(node);
            parent.children[index] = node.children[0];
        }
    });
    visit(root, (node, index, parent) => node.type === 'paragraph' && parent?.type === 'listItem', (node, index, parent) => {
        const children = node.children;
        const ids = Object.entries(children)
            .filter(([_, child]) => child.type === 'attrs')
            .map(([id, node]) => [parseInt(id), node]);
        if (!ids || ids.length === 0)
            return;
