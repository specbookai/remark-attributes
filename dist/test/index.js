...
test('fixtures with md', (t) => {
    const entries = fs
        ...
        .filter((d) => !isHidden(d) && fs.existsSync(path.join(testBase, d, 'input.md')));
...
test('fixtures with mdx', (t) => {
    const entries = fs
        .readdirSync(testBase)
        .filter((d) => !isHidden(d) && fs.existsSync(path.join(testBase, d, 'input.mdx')));
    t.plan(entries.length);
    let index = -1;
    while (++index < entries.length) {
        const fixture = entries[index];
...
test('should throw if missing end backslash and curly brace', async (st) => {
    ...
    st.throws(() => {
        renderToString(createElement(evaluateSync(file, {
            ...runtime,
            remarkPlugins: [[remarkAttributes, { mdx: true }]]
        }).default));
    }, (error) => {
